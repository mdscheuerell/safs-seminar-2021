...
class: frimg, inverse
background-image: url(figs/WA-Notify-slide.png)
background-size: 100%
...
# In 1978 the US Supreme Court ruled that

> ## The plain intent of Congress in enacting [the ESA] was to halt and reverse the trend toward species extinction, whatever the cost.
...
### Determine whether species are .red-text[threatened] or .red-text[endangered] (&sect; 4)
...
.blue-text[
## Required every 5 years to ensure that listed species have appropriate protections
...
# Outcomes of status reviews

.blue-text[
## Recommend whether a listed species should be

1. ### reclassified from threatened to endangered (uplist)

2. ### reclassified from endangered to threatened (downlist)

3. ### removed from the list (delist)

4. ### maintained at current classification
]
...
### Listed as threatened under ESA in 1999
]
...
$$\underbrace{y_t}_{\substack{\text{observed} \\ \text{counts}}} = \underbrace{x_t}_{\substack{\text{true} \\ \text{counts}}} + \underbrace{v_t}_{\substack{\text{observer} \\ \text{error}}} ~~ \text{with} ~~ v_t \sim \text{N}(0, r)$$
...
# Different survey methods have different levels of observation errors
...
### .center.green-text[ but only 6/61 were "signficantly" positive ]
...
## .blue-text[ Tom Quinn (SAFS) ]

## .blue-text[ James Losee (WDFW) ]
...
.blue-text[
## Resident fish that feed in the Salish Sea year-round until they mature and spawn
...
## Do changes in mass mirror patterns of Puget Sound Chinook salmon as a whole?
]
...
class: frimg, center
background-image: url(figs/ts_plot_both_sizes.png)
...
class: frimg, center
background-image: url(figs/ts_plot_both_sizes_fits.png)
background-position: 35% 65%
background-size: 90%

# Estimated trends in size
...
## Both data sources showed evidence for declining abundance and size over the past 40 years
]
...
### .blue-text[https://github.com/mdscheuerell/tengu]
